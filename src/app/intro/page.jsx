...
import { useOnboardingStore } from "../../store/useOnboardingStore";
import { MobileContainer } from "../../components/layout/MobileContainer";
import { login } from "../../services/auth"; // ⭐ 경로 수정
...
function IntroContent() {
  ...
  // [MOD] 실제 FastAPI 로그인 연동 및 유효성 검사 강화
  const handleLogin = async (e) => {
    ...
    try {
      const data = await login(userId, password); // [MOD] userId 변수 사용

      setUser({
        id: userId, // [MOD] email 대신 userId 저장
      });
  ...
  const handleCreateItinerary = () => router.push("/onboarding/location");
  const handleBrowse = () => router.push("/home");
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8">
          <div className="w-full flex flex-col gap-6">
            <input
              ...
              value={userId}
              onChange={(e) => setUserId(e.target.value)}
              className="w-full h-[60px] bg-[#f5f5f7] rounded-2xl px-5 outline-none focus:ring-2 focus:ring-black/5"
            />
            ...
            <button
              onClick={handleLogin}
              disabled={!userId || !password}
              className="w-full h-[56px] bg-black text-white rounded-2xl font-bold disabled:opacity-30 transition-all active:scale-[0.98]"
            >
              로그인
            </button>
